...
DEFAULT_SETTINGS = {
    ...
    "feedback_file": "palautetekstit.json",
    "language": "FIN",
    "default_paths": [],
    "title_font_size": 12,
    "normal_font_size": 10,
...
MSG = {
    ...
    "FIN": {
        ...
        "AR5-3": ("Aliohjelmakutsussa '{}', '{}' on virheellinen parametrin nimi.", ERROR),
        "AR6": ("Aliohjelman '{}' päätason lopusta puuttuu return-komento.", ERROR),
        "AR6-1": ("Käytetään generaattoria '{}' aliohjelmassa '{}'.", NOTE),
...
def has_same_parent(node, others, allowed, **kwargs):
...
def init_feedback_messages(settings) -> dict:
    ...
    messages_fin = {key:list(value) for key, value in MSG["FIN"].items()}
    ...
    MSG["FIN"] = {key:tuple(value) for key, value in messages_fin.items()}
    return None
...
class ExceptionHandlingAnalyser(ast.NodeVisitor):
   ...
   # Initialisation
    def __init__(self, model):
        ...
        self.allowed_exception_types = ALLOWED_EXCEPTION_TYPES
    ...
    def _check_exception_type(self, node):
        ...
        try:
            ...
            for i in temp:
                self.model.add_msg(
                    code="PK1-2",
                    status=(get_attribute_name(i) in self.allowed_exception_types),
                    lineno=i.lineno
                )
            temp.clear()
   ...
   # Visits
    ...
    def visit_Attribute(self, node, *args, **kwargs):
        ...
        if filehandling:
            ...
            try_node = get_parent(node, ast.Try, denied=FUNC)
        ...
        for node in tree.body:
            if hasattr(node, "value") and isinstance(node.value, ast.Call):
                ...
                try:
                    ...
                    try:
                        attr_name = get_attribute_name(func, splitted=True)
                        if (attr_name[0] in ALLOWED_LIBRARIES_FOR_CONST
...
class BasicsAnalyser(ast.NodeVisitor):
   ...
   # Grammar info
    # From: Python 3.7. version
    # Link to 3.10 version: https://docs.python.org/3.10/library/ast.html#abstract-grammar
    ...
    def _check_if_pyramid(self, node):
        ...
        # TODO change to use sibling nodes so parent is not needed.
        ...
        for _child in ast.iter_child_nodes(parent):
            if isinstance(_child, ast.If):
                try:
                    elems = get_comparator_elements(_child.test)
                except AttributeError:
                    continue

                for elem in elems:
                    try:
                        name = get_attribute_name(elem)
...
class DataStructureAnalyser(ast.NodeVisitor):
   ...
   # Initialisations
    def __init__(self, model):
        ...
        self._list_addition_attributes = LIST_ADDITION_ATTRIBUTES
...
class PreAnalyser(ast.NodeVisitor):
   ...
   # Initialisation
    def __init__(self, library=None):
        ...
        self._list_mod_attrs = LIST_MODIFICATION_ATTRIBUTES
        self._dict_mod_attrs = DICT_MODIFICATION_ATTRIBUTES
   # ------------------------------------------------------------------------- #
   # Getters
            ...
            for name in self._get_names(var, names): # name must be str or int
                if name in self.global_dict.keys():
                    ...
                    if name in self._possible_constant_dict:
                        ...
                        self._possible_constant_dict[name] = GlobalTemplate(
                            name,
                            var.lineno,
                            var
                        )
                    ...
                    self.global_dict[name] = self._possible_constant_dict.pop(name, None)
                        ...
                        and get_parent(var, CLS_FUNC) is not None):
                    self.local_global_dict[name] = NodeTemplate(
                        name,
                        var.lineno,
                        var
                    )
...
class HelpPage(tk.Frame):
    ...
    def __init__(self, parent, controller, settings):
        ...
        help_label = tk.Label(self, text=GUI[self.LANG]["not_ready_note"])
        help_label.pack()
